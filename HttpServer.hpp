...
#include "Pool.hpp"  //模板函数或模板类不能分离编译,链接器对于重复实例化代码会优化为一份,不会报符号冲突的错误
...
class HttpServer {
    private:
        const int port;  //需要监听的端口
        const int threadNum;  //线程数量
        Pool<int> fdPool;  //请求池
        ...
        //管道破裂的信号的处理函数,注意，信号处理函数若不同线程都注册了，那么信号到来时系统会随机选择一个线程进行处理,此处只用在主服务中注册即可
        static void sigpipeHandler(int sig);  

        //更新错误码对应的文件缓存,会读取目录下的所有数字.html文件,然后添加为map[数字]=数字.html内容
        ...
        //请求处理函数的调度器
        void scheduler(HttpRequest& request, HttpResponse& response);  

    public:
        explicit HttpServer(int port = 80, int threadNum = 7);

        //增加处理函数
        void addHandler(std::function<bool(const HttpRequest&, HttpResponse&)> requestHandler); 
