...
extern thread_local LogOutput logOutput;
...
void HttpRequest::fillData(int fd) {
    ...
    logOutput.dfmt("content of raw request for socket(fd=%d,size of request content=%zu):", fd, fdBuf.size());
    logOutput.d(fdBuf.c_str());  // TODO: 二进制请求可能会发生0截断,例如POST上传文件时
